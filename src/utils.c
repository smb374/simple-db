...
i32 ffz(u64 input) { return ffsll(~input) - 1; }
...
// Hardware-accelerated CRC-32C (if available)
#if defined(__x86_64__) && defined(__SSE4_2__)
#include <nmmintrin.h>

static inline u32 crc32c_hw(const u8 *data, size_t len) {
    u32 crc = 0xFFFFFFFF;

    // Process 8 bytes at a time
    while (len >= 8) {
        crc = _mm_crc32_u64(crc, *(u64 *) data);
        data += 8;
        len -= 8;
    }
...
// Software fallback CRC-32C (Castagnoli polynomial: 0x1EDC6F41)
static u32 crc32c_sw(const u8 *data, size_t len) {
    u32 crc = 0xFFFFFFFF;
    for (size_t i = 0; i < len; i++) {
...
// Auto-select hardware or software implementation
